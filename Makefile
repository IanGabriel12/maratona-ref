...
fast: | build
	$(LATEXCMD) content/ufrn.tex </dev/null
	cp build/ufrn.pdf ufrn.pdf

ufrn: build
	$(LATEXCMD) content/ufrn.tex && $(LATEXCMD) content/ufrn.tex
	cp build/ufrn.pdf ufrn.pdf

clean:
	cd build && rm -f ufrn.aux ufrn.log ufrn.tmp ufrn.toc ufrn.pdf ufrn.ptc

veryclean: clean
	rm -f ufrn.pdf test-session.pdf

.PHONY: help fast ufrn clean veryclean
